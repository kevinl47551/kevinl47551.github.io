QUESTION 1
when you enter a url in your browser it will send a get request with the url you typed in. The server will respond back with the html and it contains a link to the css and an image so it will also do get requests for those and the server will respond with the css and png file.

QUESTION 2
The environment provided on github only has the application like html and stuff and docker containers have the necessary dependencies and configurations along with your application.
_________________________________________________________________________________________________________
HOMEWORK 2

QUESTION 1
flex direction: row makes it so that the items are placed horizontaly and the main axis is horizontal and when you do something like justify content that is based on the horizontal main axis
flex direction: column (same thing as row but replace horizontal with verticle)

QUESTION 2
It is important to use relative fields because if you use fixed pixels and your screen size changes something might take up the whole screen and itll look too big but if you use relative fields itll adapt to the size of the screen or parent element. If you have a smaller screen the elements will also shrink.